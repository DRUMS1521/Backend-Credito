...
class ListPaymentsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            query_ruta = self.request.query_params.get('responsable', None)
            ...
            if query_ruta is not None and query_ruta != '':
                #use Q to filter the main queryset
                queryset = queryset.filter(Q(responsable=query_ruta))
            if query_solo_pendientes == 'true':
                queryset = queryset.filter(Q(credito__credito_finalizado=False))
        else:
            queryset = Payments.objects.filter(responsable=user, fecha_pago=timezone.now(), credito__credito_finalizado=False).order_by('id')
        return queryset
